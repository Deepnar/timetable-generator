...
- **Scale battle test** — `scripts/seed_demo.py` + `scripts/battle_test.py` + `scripts/api_drive.py` + `scripts/async_drive.py` verify the engine against a 12-department TCET-style college — **16 classes per department (FE/SE/TE/BE × 4 divisions, one semester per year)**: 288 subjects / 345 faculty / 192 groups / 324 rooms / 1152 assignments / 204 profiles. The per-class DIVISION profiles each attach exactly one group, so generation produces 192 clean per-class timetables instead of merged whole-department grids. Greedy spreads all 288 sessions of a whole-department profile across all 6 days × 8 slots (~2-4s per department); OR-Tools places a per-year profile; the real Celery worker + Redis async path, the generation lock, and cross-timetable safety were all exercised live. Surfaced and fixed three scale bugs (multi-group PDF `LayoutError`; missing `run_duration_ms` on `GenerationResponse`; greedy packing classes into minimum days instead of spreading across the week) — see DD-020.
