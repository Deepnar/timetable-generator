...
- **Frontend depth** — shipped: Auth + Dashboard + Resource CRUD (with drill-down navigation), Generation trigger, Instances list, the TimetableGrid viewer with exports, **compare mode** and the **slot override editor** (§4.1). Remaining (plan.md Phase 4): assignment grid, profile/constraint builder, and the teacher/student portals (DD-022).
...
5. ~~**Frontend (first slice)**~~ — ✅ done (Auth + Dashboard + Resource CRUD, §4.1). Remaining UI: assignment grid, profile/constraint builder, teacher/student portals (plan.md Phase 4).
