...
CREATE TABLE subject_assignments (
    ...
    tutorial_hours INTEGER                              -- of weekly_hours, how many are TUTORIAL sessions
                                                         -- (Phase 5, DD-046; NULL = no tutorial stream)
);
...
GET    /notifications                  The caller's in-app notifications, newest first (DD-027);
                                        # ?unread_only=true, X-Total-Count pagination.
GET    /notifications/unread-count     How many unread notifications the caller has (the bell badge).
...
5. **Solver Runs N times for N candidate instances.** The run row records a `variation` strategy (from the request, default `random`) that shapes the seeded re-rolls (§5.3):
   ...
   - For each attempt (up to `_DIVERSITY_ATTEMPTS = 6`): greedy shuffles `working_days` / `slot_times` / `rooms` — or reorders its search by the gap criterion for `minimize-teacher-gaps` / `minimize-student-gaps`; OR-Tools varies `solver.parameters.random_seed` and, for a gap criterion, adds a small secondary objective term (§5.2).
    - **Acceptance (Phase 4, A6):** keep the first fingerprint whose Hamming distance from every already-accepted instance is ≥ `_DIVERSITY_MIN_DISTANCE = 1`; otherwise try the next seed. If all 6 attempts collide, the **last attempt** is kept (so a tiny problem still produces a result rather than a duplicate). **Quality is the default**: whenever at least one active soft rule exists, every attempt is scored and the **highest-scoring distinct attempt** is kept — for `variation="best"` *and* the default path — so the scheduler no longer takes the first different timetable. Without soft rules there is nothing to score and the first distinct attempt wins (diversity semantics unchanged).
...
- **`Scheduler._load_published_conflicts()`** (`app/engine/scheduler.py`) — at the start of generation, it queries every `PUBLISHED` `timetable_instance` across all generations and builds three `set[ResourceKey]` per resource kind:
  ...
  - **Phase 4 (A4):** it also returns per-faculty load counts — `faculty_day_counts` (faculty → day → slot-count) and `faculty_week_counts` (faculty → slot-count) — so the faculty caps measure candidate + committed + published load and stay honest across runs.
...
- **Schema (25 tables)** — Alembic chain `aeaadc4f2374 → … → c9d4e8f2a6b0`; latest migration is `d4e8f2a6c0b1` (assignment `tutorial_hours`).
...
- **Two-stream subjects (Phase 5, DD-046)** — `subject_assignments.tutorial_hours` splits a subject's weekly load into lecture-type sessions + TUTORIAL sessions (exempt from SAME_SUBJECT_SAME_DAY); the importer reads the split from the grid's TUTORIAL cells. IT-SE-C unplaced 4 → 0.
- **Fidelity + golden tests (Phase 5, A8/D4/D3)** — `app/engine/fidelity.py` scores a generated timetable against the published grids (hours deltas, break/Saturday violations, room stability, teacher counts, utilisation, batch coverage, grid-side gaps); `scripts/synthetic_problem.py` plants valid timetables and derives the solver inputs (any unplaced session is provably a solver bug; the D3 second-fixture shape — no home rooms, 2-slot labs — solves with zero engine changes); `scripts/golden_test.py` regenerates every division and fails on regression (COMP+IT: 21/21 zero unplaced, 0 solver-attributable hour misses).
- **Constraint engine** — `HARD_CONSTRAINT_REGISTRY` + `SOFT_CONSTRAINT_REGISTRY`; rules are **tiered** (INVARIANT / INSTITUTIONAL / OPTIONAL / PREFERENCE, Phase 3b A10): invariant physics (double-booking, room requirements, availability, blackouts, cross-timetable safety, break slots, lab-rotation integrity) is always-on, while institutional policy (`SAME_SUBJECT_SAME_DAY`, `MAX_ONE_LAB_PER_DAY`, `CROSS_DEPT_DAILY_CAP`, room capacity, faculty load caps) fires only from a profile/college-default row — the migration + importer seed the defaults that preserve pre-Phase-3b behaviour, and a registrar tunes them through the API. Optional rule-pack rules (`SUBJECT_TIME_PREFERENCE`, `MAX_CONSECUTIVE_SAME_TEACHER`, `MAX_DAILY_SUBJECTS`, `TEACHER_YEAR_RESTRICTION`, `LAB_BATCH_ROTATION`, `HOLIDAY_CALENDAR`, `EXAM_DATE_SEPARATION`, `CONTIGUOUS_LAB_SLOTS`) stay per-profile opt-in. All six soft types (`TEACHER_PREFERS_MORNING`, `AVOID_CONSECUTIVE_SAME_SUBJECT`, `MINIMIZE_STUDENT_FREE_SLOTS`, `MINIMIZE_TEACHER_FREE_SLOTS`, `DISTRIBUTE_SUBJECTS_EVENLY`, `BALANCE_TEACHER_LOAD`) ship scorers + CP-SAT objective builders, and the greedy solver pursues them during placement via a preference-aware scan. A startup assertion (`assert_registry_enum_parity`) pins the registry to the `ConstraintType` enum, and `GET /constraints/types` returns tier + config JSON-schema per type. Phase 4 (A6/A4): `is_valid` is fail-fast, the greedy order is most-constrained-first, quality (best-scoring distinct attempt) is the default when soft rules exist, and the faculty caps include published load.
- **Lab windows (Phase 2 A1 + DD-044)** — a window is `(group, period_number)`; members share `window_key`. The cell parser reads lab faculty positionally (between the batch pattern and the room) so a pair's second initial is never dropped, and window members that share (subject, faculty) — one teacher for a batch pair — merge into a single session covering the pair, so the window co-locates. The batch<->subject rotation is constructed as a Latin square, never searched.
...
- **Two-channel notifications (DD-027)** — publish and mid-year changes notify in-app + email. A new `app_notifications` table holds one row per recipient Admin (resolved by email from the schema links: admin/hod accounts, the instance's faculty, linked group `incharge_email`/`student_email`, and affected teachers for a change); `notification_service.dispatch_publish` / `dispatch_change` run after the publish/override commits, best-effort. Email reuses the publish mailer (§7.7) and adds a compact change email; a mail outage never fails the request. The topbar bell polls unread count and `/notifications` lists/marks rows (§4.1).
...
- **Frontend depth** — shipped: Auth + Dashboard + Resource CRUD (with drill-down navigation), Generation trigger, Instances list, the TimetableGrid viewer with exports, **compare mode**, the **slot override editor**, the **assignment grid**, the **profile/constraint builder**, the **mid-year change mode** on published timetables, the **role portals** (`/my-schedule` teacher + `/my-timetable` student — Today card + weekly grid + own exports, with role-based login redirect), and the **notification bell + /notifications page** (§4.1). Remaining (plan.md Phase 4): the date-resolution day layer (DD-022 #2/#3).
