...
- [x] **Implement New Constraint Types** *(8 of 8 done)*
  ...
  - [x] `MAX_DAILY_SUBJECTS`: Cap how many distinct subjects a group sits per day (`config_json.max`); modelled relationally in OR-Tools.
...
- [x] **Soft Constraint Scoring** *(done, solver-independent)*
  - `app/engine/scorer.py` registry scores each instance's soft constraints (weighted mean → `instance.soft_score`, best → `generation.score_best_instance`), gated by `enable_soft_constraint_scoring`. Ships scorers + CP-SAT builders for all six catalogued types (`TEACHER_PREFERS_MORNING`, `AVOID_CONSECUTIVE_SAME_SUBJECT`, `MINIMIZE_STUDENT_FREE_SLOTS`, `MINIMIZE_TEACHER_FREE_SLOTS`, `DISTRIBUTE_SUBJECTS_EVENLY`, `BALANCE_TEACHER_LOAD`); the greedy solver now also pursues them during placement via a preference-aware (day, slot) scan.
...
- [x] **Profile & Constraint Builder**: visual form for profiles and dynamic constraints.
  ...
  - **Remaining:** WebSocket push / live delivery (DD-027 follow-up). Portals shipped: `/my-schedule` (teacher) and `/my-timetable` (student) — role-based login redirect, day card, weekly grid, own exports via `GET /my/schedule` / `/my/timetable` / `/my/today` / `/my/export/{pdf,csv,ical}`; teacher identity by email→Faculty match, student identity by `student_groups.student_email`. The **date-resolution layer** (`?date=` + `/my/today`) resolves mid-year changes per date (DD-022 #2). Seed provisions linked demo logins.
...
- [x] **RBAC** — `Admin.role` (`admin`/`hod`/`teacher`/`student`), JWT role claim, `require_roles(...)` dependency, `GET /auth/me`, admin-only `POST /auth/users` (DD-021). Teacher/student read-scoping is a follow-up.
...
- [x] **Full-college timetable (demo seed goal)** — `scripts/generate_college.py` generates + publishes a timetable for the entire **fabricated demo** college (12/12 demo departments, 3456 slots in the local DB). The real-data rollout — replacing the demo departments/FE scheme/strengths/rooms with the actual TCET structure from `info/` — is `documentation/real-data-rollout-plan.md` and is the pre-launch task.
- [x] **Security audit (DD-029)** — v4-pro deepscan remediated: role gates on every admin router, least-privilege self-registration, hardened error/upload/header surfaces, `/generate` rate limit, JWT-expiry check. Accepted items (httpOnly-cookie JWT, psycopg2-binary, Next/React patch bumps) tracked as follow-ups.
- [x] **Scale battle test** — `scripts/seed_demo.py` seeds a 12-department **fabricated demo** college; `scripts/battle_test.py` / `api_drive.py` / `async_drive.py` verify greedy (whole-dept 288 sessions in ~4.3s), OR-Tools (per-semester 36 sessions), the async Celery path, the generation lock, and cross-timetable safety against live Postgres/Redis. Two scale bugs found and fixed (multi-group PDF, `run_duration_ms`). See DD-020.
