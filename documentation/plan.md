...
- [x] **Profile & Constraint Builder**: visual form for profiles and dynamic constraints.
  ...
  - **Remaining:** date-resolution day layer (DD-022 #2/#3). Portals shipped: `/my-schedule` (teacher) and `/my-timetable` (student) — role-based login redirect, Today card, weekly grid, own exports via `GET /my/schedule` / `/my/timetable` / `/my/today` / `/my/export/{pdf,csv,ical}`; teacher identity by email→Faculty match, student identity by `student_groups.student_email`. Seed provisions linked demo logins.
