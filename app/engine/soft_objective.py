...
def _minimize_student_free_slots(ctx: ObjectiveContext, config) -> list:
    ...
    by_group_day: dict[tuple, list] = {}
    for (si, day, sn, _room), var in x.items():
        gid = sessions[si].student_group_id
        by_group_day.setdefault((gid, day), []).append((sn, var))

    terms: list = []
    for (gid, day), placements in by_group_day.items():
        # A single placement has no gaps; anything more needs span modeling.
        if len(placements) < 2:
            continue
        has = model.NewBoolVar(f"compact_has_{gid}_{day}")
        model.Add(has <= sum(v for _, v in placements))
        for _, var in placements:
            model.Add(has >= var)
        first = model.NewIntVar(1, slot_count, f"compact_first_{gid}_{day}")
        last = model.NewIntVar(1, slot_count, f"compact_last_{gid}_{day}")
        span = model.NewIntVar(0, slot_count, f"compact_span_{gid}_{day}")
        for sn, var in placements:
            model.Add(first <= sn + slot_count * (1 - var))
            model.Add(last >= sn * var)
        # span == last - first + 1 when the group-day is occupied, else 0.
