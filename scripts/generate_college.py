"""Generate and publish a timetable for the ENTIRE college.
...
published timetable covering every department — one whole-department instance
per department, all semesters and divisions at once. After it runs, the
teacher and student portals show real published schedules and the exports /
change-mode / notifications surfaces have live data.

Usage:
    uv run python -m scripts.seed_demo --wipe   # fresh dataset (server NOT needed)
    uv run python -m scripts.generate_college [--instances 3] [--dry-run]

For each DEPARTMENT-scoped profile it runs greedy with ``--instances``
variations (default 3, best-wins via ``variation="best"``), picks the
...
def main() -> int:
    ...
    parser.add_argument("--instances", type=int, default=3,
                        help="instances per department (best-wins variation)")
    parser.add_argument("--dry-run", action="store_true",
                        help="generate but do not publish")
    parser.add_argument("--only", type=str, default=None,
                        help="run one department by name fragment")
    ...
    try:
        ...
        profiles = db.scalars(
            select(TimetableProfile).where(
                TimetableProfile.scope_type == "DEPARTMENT"
            ).order_by(TimetableProfile.department)
        ).all()
        if args.only:
            profiles = [p for p in profiles if args.only.lower() in p.department.lower()]
        if not profiles:
            raise SystemExit(f"no DEPARTMENT profiles found (only={args.only})")

        print(f"Generating the full college timetable "
              f"({len(profiles)} departments, {args.instances} instances each)…")
        print()
        ...
        for prof in profiles:
            ...
            results.append(row)
            print(f"  {row['department'][:38]:<38} gen {row['generation']:>3} "
                  f"inst {row['instances']:>2} slots {row['slots']:>4} "
        ...
        print(f"Completed {len(ok)}/{len(results)} departments; "
              f"{len(published)} published.")
        failed = [r for r in results if r["status"] != "COMPLETED"]
        if failed:
            print("FAILED:")
            for r in failed:
                print(f"  {r['department']}: {r['error']}")
            return 1
        return 0
    finally:
        db.close()
