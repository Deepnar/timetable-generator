...
import type { MyTimetableResponse, MyTodayResponse } from "@/lib/types";
...
export default function MyTimetablePage() {
  ...
  // Real time grid from the group timetable's slot start times.
  const timeGrid = useMemo(() => {
    const byNumber = new Map<number, string>();
    for (const s of timetable.data?.slots ?? []) {
  ...
  const sessions = useMemo<GridSession[]>(
    () =>
      (today.data?.slots ?? []).map((s) => ({
        ...
        facultyName: s.faculty_name ?? undefined,
        day: s.day_of_week ?? 0,
  ...
  async function download(ext: string) {
    try {
      ...
      a.href = url;
      a.download = `my-timetable.${ext === "ical" ? "ics" : ext}`;
      a.click();
    ...
    <ProtectedShell>
      <div className="flex flex-col gap-5">
        <div className="flex items-end justify-between">
          <div>
            <h1 className="display text-3xl text-ink">My timetable</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              {me?.name} · {groupName ?? "Student"} · published timetable
            </p>
          </div>
        ...
        {(timetable.isError || today.isError) && (
                ...
                <li key={s.id} className="flex items-center justify-between py-2">
                  ...
                  <div className="text-right text-sm text-ink-soft">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {s.faculty_name}{s.room_code ? ` · ${s.room_code}` : ""}
                    </p>
                  </div>
                </li>
              ))}
        ...
        <div className="rounded-md border bg-surface p-4 shadow-sm">
          ...
          ) : timetable.data?.group == null ? (
            <EmptyState
              icon={CalendarDays}
              title="No group linked to your account"
              body="Your login email isn't linked to a student group yet. Contact the admin to link it."
            />
          ) : sessions.length === 0 ? (
            <EmptyState
              icon={CalendarDays}
